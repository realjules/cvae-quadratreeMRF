...
# -*- coding: utf-8 -*-
"""
Script to train the CrossScaleMRF model with cross-scale attention for better 
...
class SegmentationTrainer:
    """
    Trainer class for CrossScaleMRF segmentation model.
    This class handles:
    - Training with fixed pre-trained CVAE encoder
    ...
    def __init__(self, args):
        ...
        self.output_dir = f"output/cross_scale_mrf_{timestamp}"
    ...
    def _init_models(self):
        ...
        # CVAE model (fixed, pre-trained encoder)
        ...
        if self.args.cvae_weights and os.path.exists(self.args.cvae_weights):
            ...
            print(f"Loaded pre-trained CVAE weights from {self.args.cvae_weights}")
        else:
    ...
    def _extract_cvae_features(self, images):
        """Extract features from CVAE encoder"""
        with torch.no_grad():
...
def main():
    parser = argparse.ArgumentParser(description="Train CrossScaleMRF Model")
    ...
    parser.add_argument('--cvae_weights', type=str, default=None, help='Path to pre-trained CVAE weights')
