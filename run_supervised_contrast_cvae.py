...
# -*- coding: utf-8 -*-
"""
Script to train the Supervised Contrastive CVAE component with higher contrastive weight 
and supervised contrastive loss when labels are available
"""
...
from net.supervised_contrast_cvae import SupervisedContrastCVAE
...
class SupervisedContrastCVAELoss(torch.nn.Module):
    """
    Enhanced CVAE loss function with:
    - Reconstruction loss (MSE + SSIM)
    - KL divergence
    - Stronger contrastive loss (higher weight)
    - Supervised contrastive loss when labels are available
    - Perceptual loss (if available)
    """
    def __init__(self, kld_weight=0.0001, contrastive_weight=1.0, supervised_weight=1.0, 
                 ssim_weight=0.05, perceptual_weight=0.01):
        super(SupervisedContrastCVAELoss, self).__init__()
        # Using higher contrastive weight
    ...
    def forward(self, outputs, targets=None, mode='full'):
        ...
        z_proj_sup = outputs.get('z_proj_sup', None)
        original_input = outputs['original_input']
        labels = outputs.get('labels', None)
        
        # Queues for contrastive learning
        queue = outputs['queue']
        supervised_queue = outputs.get('supervised_queue', None)
        label_queue = outputs.get('label_queue', None)
        
        # Model weights
        contrastive_weight = outputs.get('contrastive_weight', self.contrastive_weight)
        supervised_weight = outputs.get('supervised_weight', self.supervised_weight)
        ...
        adaptive_contrastive = contrastive_weight * (0.8 + 0.2 * progress)
        
        # Adaptive supervised weight - starts lower and increases faster
        adaptive_supervised = supervised_weight * (0.5 + 0.5 * progress)
        
        # Unsupervised contrastive loss
        model = outputs.get('model', None)
        if model is not None:
            contrastive_loss = model.unsupervised_contrastive_loss(z_proj)
        else:
            # Fallback implementation if model isn't provided
            contrastive_loss = torch.tensor(0.0, device=x_recon.device)
        
        # Supervised contrastive loss if labels are available
        if model is not None and labels is not None and z_proj_sup is not None:
            supervised_loss = model.supervised_contrastive_loss(z_proj_sup, labels)
        else:
            # No supervised loss if labels aren't available
            supervised_loss = torch.tensor(0.0, device=x_recon.device)
        
        # Combine all losses
        total_loss = recon_loss + \
                     kld_weight * kld_loss + \
                     adaptive_contrastive * contrastive_loss + \
                     adaptive_supervised * supervised_loss + \
...
def train_model(args):
    ...
    output_dir = f"output/supervised_contrast_cvae_{timestamp}"
    ...
    criterion = SupervisedContrastCVAELoss(
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        for batch_idx, batch in enumerate(pbar):
            ...
            if args.unsupervised:
                ...
                # Extract class labels from target masks (use most common class for simplicity)
                labels = []
                for target in targets:
                    # Flatten mask and find most common class (exclude ignore_index=6)
                    values = target.flatten()
                    values = values[values != 6]  # Remove ignore index
                    if len(values) > 0:
                        unique, counts = torch.unique(values, return_counts=True)
                        most_common = unique[counts.argmax()]
                        labels.append(most_common.item())
                    else:
                        # Default to class 0 if no valid classes
                        labels.append(0)
            ...
            if args.mixed_precision:
                with torch.cuda.amp.autocast():
                    # Forward pass
                    outputs = model(images, labels if not args.unsupervised else None)
            ...
            pbar.set_postfix({
                ...
                'contr': epoch_contrastive_loss / (batch_idx + 1),
                'super': epoch_supervised_loss / (batch_idx + 1)
            })
        ...
        avg_supervised_loss = epoch_supervised_loss / len(dataloader)
        ...
        loss_history['supervised'].append(avg_supervised_loss)
        ...
        print(f"Epoch {epoch+1}/{args.epochs} - Loss: {avg_loss:.4f}, Recon: {avg_recon_loss:.4f}, "
              ...
              f"Supervised: {avg_supervised_loss:.4f}, SSIM: {avg_ssim_loss:.4f}, "
...
def plot_loss_curves(loss_history, save_path):
    ...
    # Contrastive losses
    axes[1, 1].plot(loss_history['contrastive'], label='Unsupervised')
    axes[1, 1].plot(loss_history['supervised'], label='Supervised')
    axes[1, 1].set_title('Contrastive Losses')
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Train Supervised Contrastive CVAE Model")
    ...
    parser.add_argument('--contrastive_weight', type=float, default=1.0, help='Weight for contrastive loss')
    parser.add_argument('--supervised_weight', type=float, default=1.0, help='Weight for supervised contrastive loss')
