...
class SupervisedContrastCVAE(nn.Module):
    """
    Enhanced Contrastive Variational Autoencoder with:
    - Increased contrastive weight
    - Stronger supervised contrastive learning
    ...
    def __init__(self, input_channels=3, latent_dim=512, hidden_dims=[64, 128, 256, 512],
                 contrastive_weight=1.0, supervised_weight=1.0):
        super(SupervisedContrastCVAE, self).__init__()
        ...
        # Increased contrastive weight (default: 1.0 instead of 0.5)
        self.contrastive_weight = contrastive_weight
        
        # New supervised weight for supervised contrastive loss
        ...
        # Enhanced projection head for contrastive learning (3-layer MLP with larger dimensions)
        ...
        # Enhanced supervised projection head (separate pathway for label-based projection)
        self.supervised_projection_head = nn.Sequential(
            nn.Linear(latent_dim, latent_dim),
            nn.BatchNorm1d(latent_dim),
            nn.ReLU(),
            nn.Linear(latent_dim, latent_dim // 2),
        ...
        # 8192 items instead of 4096 for even better representation of dataset distribution
        self.register_buffer("queue", torch.randn(8192, latent_dim // 4))
        self.queue = F.normalize(self.queue, dim=1)
        self.register_buffer("queue_ptr", torch.zeros(1, dtype=torch.long))
        
        # New: Memory bank for supervised samples with class labels
        # This stores embeddings along with their class labels
        self.register_buffer("supervised_queue", torch.randn(2048, latent_dim // 4))
        self.register_buffer("label_queue", torch.zeros(2048, dtype=torch.long))
        self.register_buffer("supervised_queue_ptr", torch.zeros(1, dtype=torch.long))
    ...
    def project(self, z, supervised=False):
        """Project latent representations for contrastive learning"""
        if supervised:
            # Use supervised projection head for labeled data
            return self.supervised_projection_head(z)
        else:
            # Use standard projection head for unlabeled data
            return self.projection_head(z)
    
    @torch.no_grad()
    def update_queue(self, z_proj):
        """Update memory bank queue for unsupervised contrastive learning"""
    ...
    @torch.no_grad()
    def update_supervised_queue(self, z_proj, labels):
        """Update supervised memory bank with projections and their labels"""
        batch_size = z_proj.shape[0]
        ptr = int(self.supervised_queue_ptr)
        
        # Replace the features and labels at ptr (dequeue and enqueue)
        if ptr + batch_size > self.supervised_queue.shape[0]:
            # Handle wrapping
            first_part = self.supervised_queue.shape[0] - ptr
            self.supervised_queue[ptr:] = z_proj[:first_part]
            self.label_queue[ptr:] = labels[:first_part]
            self.supervised_queue[:batch_size - first_part] = z_proj[first_part:]
            self.label_queue[:batch_size - first_part] = labels[first_part:]
            ptr = batch_size - first_part
        else:
            # Normal case
            self.supervised_queue[ptr:ptr + batch_size] = z_proj
            self.label_queue[ptr:ptr + batch_size] = labels
            ptr = (ptr + batch_size) % self.supervised_queue.shape[0]
        
        self.supervised_queue_ptr[0] = ptr
    ...
    def forward(self, x, labels=None):
        """Forward pass through the Enhanced CVAE with supervised contrastive options"""
        ...
        # Project for supervised contrastive learning if labels are provided
        if labels is not None:
            z_proj_sup = self.project(z, supervised=True)
            z_proj_sup_norm = F.normalize(z_proj_sup, dim=1)
        else:
            z_proj_sup_norm = None
        
        # Update memory banks (only during training)
        if self.training:
            with torch.no_grad():
                # Update unsupervised memory bank
                self.update_queue(z_proj_norm.detach())
                
                # Update supervised memory bank if labels are provided
                if labels is not None:
                    self.update_supervised_queue(z_proj_sup_norm.detach(), labels)
        ...
        return {
            ...
            'z': z,
            'z_proj': z_proj_norm,
            'z_proj_sup': z_proj_sup_norm if labels is not None else None,
            'original_input': x,
            'queue': self.queue,
            'supervised_queue': self.supervised_queue if labels is not None else None,
            'label_queue': self.label_queue if labels is not None else None,
    ...
    def unsupervised_contrastive_loss(self, z_proj, temperature=0.05):
        """Enhanced unsupervised contrastive loss with larger memory bank and harder negatives"""
        ...
        # Calculate similarities with larger queue (now 8192 items)
        queue_sim = torch.mm(z_proj_norm, queue.t()) / temperature
        
        # Apply harder negative mining - identify difficult negatives
        ...
        with torch.no_grad():
            ...
            # Remove self-similarity and keep more hard negatives (33% instead of 25%)
            hard_indices = hard_indices[:, 1:int(batch_size * 0.33) + 1]  # Top 33% hardest
        ...
        for i in range(batch_size):
            # Increase weight for hard negatives (now 3x weight instead of 2x)
            hard_weights[i, hard_indices[i]] = 3.0
        ...
        # Return loss multiplied by the increased contrastive weight
        return loss * self.contrastive_weight
    
    def supervised_contrastive_loss(self, z_proj, labels, temperature=0.05):
        """Enhanced supervised contrastive loss with improved memory bank support"""
        if labels is None or z_proj is None:
            return torch.tensor(0.0, device=self.queue.device)
        ...
        sim_matrix = torch.matmul(z_proj_norm, z_proj_norm.t()) / temperature
        ...
        # Include supervised memory bank if available
        supervised_queue = self.supervised_queue.clone().detach()
        supervised_labels = self.label_queue.clone().detach()
        
        # Compute expanded positive mask including memory bank
        queue_pos_mask = torch.zeros(batch_size, supervised_queue.shape[0], device=device)
        
        # Match labels between current batch and queue
        for i in range(batch_size):
            queue_pos_mask[i] = (labels[i] == supervised_labels).float()
        
        # Calculate similarities with memory bank
        queue_sim = torch.matmul(z_proj_norm, supervised_queue.t()) / temperature
        
        # Expanded similarity matrix
        sim_matrix_expanded = torch.cat([sim_matrix, queue_sim], dim=1)
        
        # Expanded positive mask
        pos_mask_expanded = torch.cat([pos_mask, queue_pos_mask], dim=1)
        
        # Count number of positives for each sample
        num_pos = pos_mask_expanded.sum(dim=1)
        ...
        log_prob = torch.log(exp_sim / exp_sim_sum + 1e-10)
        
        # Apply focal loss modification to emphasize harder examples
        # Now with stronger focal weight (cube instead of square)
        focal_weight = (1 - torch.exp(log_prob)) ** 3  # cubed focal term
        weighted_log_prob = focal_weight * log_prob
        
        # Compute final supervised loss
        mean_log_prob_pos = (pos_mask_expanded * weighted_log_prob).sum(1) / num_pos.clamp(min=1)
        loss = -mean_log_prob_pos[valid_samples].mean()
        
        # Return loss multiplied by the supervised weight
        return loss * self.supervised_weight
