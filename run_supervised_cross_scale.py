...
# -*- coding: utf-8 -*-
"""
Script to train the integrated model combining SupervisedContrastCVAE with CrossScaleMRF
This combines the supervised contrastive learning with higher contrastive weight
with cross-scale attention for MRF feature integration.
"""
...
class IntegratedSegmentationTrainer:
    ...
    This class handles:
    - Training with SupervisedContrastCVAE + CrossScaleMRF
    ...
    def __init__(self, args):
        ...
        self.output_dir = f"output/supervised_cross_scale_{timestamp}"
    ...
    def _init_models(self):
        ...
        # SupervisedContrastCVAE model (fixed, pre-trained encoder)
        self.cvae = SupervisedContrastCVAE(
            input_channels=3,
            latent_dim=self.args.latent_dim,
            hidden_dims=[64, 128, 256, 512],
            contrastive_weight=self.args.contrastive_weight,
            supervised_weight=self.args.supervised_weight
        ).to(self.device)
        ...
        if self.args.cvae_weights and os.path.exists(self.args.cvae_weights):
            ...
            print(f"Loaded pre-trained SupervisedContrastCVAE weights from {self.args.cvae_weights}")
        else:
    ...
    def _extract_cvae_features(self, images, targets=None):
        """Extract features from SupervisedContrastCVAE encoder with label support"""
        with torch.no_grad():
            # Extract class labels from target masks for supervised features
            labels = None
            if targets is not None:
                ...
                labels = torch.tensor(labels, device=images.device, dtype=torch.long)
            ...
            # Encode with CVAE - passing labels for supervised features when available
            if labels is not None:
                # For training, use supervised features when possible
                mu, log_var, encoder_features = self.cvae.encode(images_with_pos)
            else:
                # For inference, use standard features
                mu, log_var, encoder_features = self.cvae.encode(images_with_pos)
    ...
    def train_epoch(self, epoch):
        ...
        for batch_idx, (images, targets) in enumerate(pbar):
            ...
            # Extract features from CVAE with label support
            features, _ = self._extract_cvae_features(images, targets)
...
def main():
    parser = argparse.ArgumentParser(description="Train Integrated SupervisedContrastCVAE + CrossScaleMRF Model")
    ...
    parser.add_argument('--cvae_weights', type=str, default=None, help='Path to pre-trained SupervisedContrastCVAE weights')
    parser.add_argument('--class_weights', action='store_true', help='Use class weights for loss calculation')
    parser.add_argument('--contrastive_weight', type=float, default=1.0, help='Contrastive weight for CVAE')
    parser.add_argument('--supervised_weight', type=float, default=1.0, help='Supervised contrastive weight for CVAE')
